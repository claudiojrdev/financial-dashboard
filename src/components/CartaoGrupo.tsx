  ...
  return (
    <div>
      <button
        ...
        style={{ borderLeftColor: grupo.catCor }}
      >
        <div className="flex flex-wrap items-center gap-x-1.5 gap-y-0.5">
          ...
          />
          <span className="break-words font-medium text-slate-700 dark:text-slate-200">
            {grupo.catNome}
          </span>
          <span className="shrink-0 text-[10px] text-slate-400">
            {grupo.contas.length} {grupo.contas.length === 1 ? 'mov.' : 'mov.'}
          </span>
          <span className="shrink-0 tabular-nums font-semibold text-slate-600 dark:text-slate-300">
