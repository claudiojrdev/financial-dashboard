...
function getConfig(userId) {
  const db = getDB();
  const linhas = db.prepare('SELECT chave, valor FROM config_usuario WHERE user_id = ?').all(userId);
  return Object.fromEntries(linhas.map((r) => [r.chave, r.valor]));
}

function mapearCategoria(db, nome, meeventos_id, userId) {
...
routerSync.post('/meeventos/sync', async (req, res) => {
  try {
    const config = getConfig(req.userId);
    ...
    db.prepare('DELETE FROM sync_meta WHERE chave = ?').run(`sync_erro_${req.userId}`);
    ...
    do {
      ...
      const upsertMov = db.prepare(`
        INSERT INTO movimentacoes (id, nome, categoria_id, unidade, tipocobranca, valor, valor_pago, data_vencimento, data_pagamento, pago, meeventos_id, meeventos_evento, user_id, updated_at)
        VALUES (@id, @nome, @categoria_id, @unidade, @tipocobranca, @valor, @valor_pago, @data_vencimento, @data_pagamento, @pago, @meeventos_id, @meeventos_evento, @user_id, @agora)
      ...
      const insertBatch = db.transaction((items) => {
        for (const item of items) {
          ...
          mov.categoria_id = mapearCategoria(db, item.categoria, idCat, req.userId);
          upsertMov.run({ ...mov, user_id: req.userId, agora: agoraSQL });
    ...
    if (idsSincronizados.length > 0) {
      ...
      db.prepare(`DELETE FROM movimentacoes WHERE user_id = ? AND id LIKE 'meev-%' AND id NOT IN (${placeholders})`).run(req.userId, ...idsSincronizados);
    }

    const agora = new Date().toISOString();
    db.prepare('INSERT INTO sync_meta (chave, valor) VALUES (?, ?) ON CONFLICT(chave) DO UPDATE SET valor = excluded.valor')
      .run(`ultima_sync_${req.userId}`, agora);

    res.json({ status: 'ok', movimentos_sincronizados: total, ultima_sync: agora });
  } catch (err) {
    const db = getDB();
    db.prepare('INSERT INTO sync_meta (chave, valor) VALUES (?, ?) ON CONFLICT(chave) DO UPDATE SET valor = excluded.valor')
      .run(`sync_erro_${req.userId}`, err.message);
    res.status(500).json({ erro: err.message });
  }
});

routerSync.get('/meeventos/status', (req, res) => {
  const db = getDB();

  const metas = db.prepare('SELECT chave, valor FROM sync_meta WHERE chave IN (?, ?)').all(
    `ultima_sync_${req.userId}`,
    `sync_erro_${req.userId}`
  );
  const status = Object.fromEntries(metas.map((r) => [r.chave, r.valor]));

  const total = db.prepare('SELECT COUNT(*) as total FROM movimentacoes WHERE user_id = ?').get(req.userId);

  const config = getConfig(req.userId);

  res.json({
    configurado: !!config.meeventos_url,
    ultima_sync: status[`ultima_sync_${req.userId}`] || null,
    sync_erro: status[`sync_erro_${req.userId}`] || null,
